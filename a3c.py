...
class ActorNetwork(object):
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            #dense_net_0 = tflearn.fully_connected(merge_net_1, 128, activation='relu')
...
def compute_gradients(epoch, s_batch, a_batch, r_batch, terminal, actor, critic):
    """
    ...
    actor_gradients = actor.get_gradients(s_batch, a_batch, td_batch, epoch)
...
def build_summaries():
    ...
    avg_test = tf.Variable(0.)
    tf.summary.scalar("Avg_test", avg_test)
    summary_vars = [td_loss, eps_total_reward, avg_entropy, avg_test]
